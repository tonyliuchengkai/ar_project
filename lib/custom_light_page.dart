...
class _CustomLightPageState extends State<CustomLightPage> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Custom Light Sample')),
    body: ARKitSceneView(
  ...
  void _addLight(ARKitController controller) {
    final light = ARKitLight(type: ARKitLightType.spot, color: Colors.blueGrey);
    final node = ARKitNode(
      light: light,
      position: vector.Vector3(0.1, 0.4, 0.5),
    );
    controller.add(node);
  }
