...
class _RealTimeUpdatesPageState extends State<RealTimeUpdatesPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Real Time Updates Sample')),
      body: ARKitSceneView(onARKitViewCreated: _onARKitViewCreated),
    );
  }
      ...
      if (busy == false) {
        ...
        this.arkitController.performHitTest(x: 0.25, y: 0.75).then((results) {
          if (results.isNotEmpty) {
            ...
            final newNode = ARKitNode(geometry: sphere, position: position);
